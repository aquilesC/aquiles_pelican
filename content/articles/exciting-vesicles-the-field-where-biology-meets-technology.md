...
Summary: Extracellular Vesicles are a fascinating topic of research in biology and biomedical sciences. These particles, released by all cells, are fundamental agents for intracellular communication in complex organisms.
---
...
Coming from a [purely physics background](https://notes.aquiles.me/my_phd/), my interest in extracellular vesicles is somewhat tangential to the tools used for their characterization. During the past 4+ years, I have been developing technologies to measure their concentration, size, refractive index, and protein content.

When [Dispertech](https://notes.aquiles.me/dispertech/) started, we focused on extracellular vesicles because we believed it was a booming field, with plenty of opportunities for new analytical tools (although we may have misjudged what the field needed.)
...
Today, a broad pool of techniques has converged on the field of extracellular vesicles. On the one hand, sample preparation is mostly focused on _enriching_. This means increasing the concentration of interesting particles, and removing as much as possible of the background.

Techniques such as [size exclusion chromatography](https://notes.aquiles.me/size_exclusion_chromatography_is_the_second_most_popular_ev_enrichment_method/) ([SEC](https://notes.aquiles.me/sec/)), or [tangential-flow-filtration](https://notes.aquiles.me/tangential-flow-filtration/) separate particles based on their size. [Separating EVs based on density](https://notes.aquiles.me/separating_evs_based_on_density/) through [density gradient centrifugation](https://notes.aquiles.me/density_gradient_centrifugation/) is also possible, and there’s quite some enthusiasm on using [free flow electrophoresis](https://notes.aquiles.me/free_flow_electrophoresis/), which sorts particles based on _mobility_.

However, none of those methods is perfect. In biological samples there are always overlapping properties, therefore separation based on only 1 physical dimension seems almost impossible. That is why preparing samples requires a combination of steps, including ultra-centrifugation, filtration, etc. The hope is that the _few_ remaining particles in the vial are going to be those of interest.
...
The majority of the extracellular vesicles in the body are very small, with diameters well below

. This range is impossible to measure with conventional optical microscope, since they will all look like bright blobs. Measuring size, therefore, requires a more sophisticated approach.

#### NTA

[Nanoparticle tracking analysis](https://notes.aquiles.me/nanoparticle_tracking_analysis/) (NTA) is a smart way of using a conventional microscope to measure the size of _colloids_, particles that are free to diffuse in a liquid. By following the movement of individual particles, it is possible to determine their diffusion coefficient, and then relating that to each particle’s size.

The [limitations of nanoparticle tracking analysis](https://notes.aquiles.me/limitations_of_nanoparticle_tracking_analysis/) include that small particles are very hard to characterize. Bellow

particles diffuse too quickly and are very dim. This means that on the microscope’s camera they are very hard to localize, yielding inaccurate size measurements.

Sample confinement through the use of a [hollow optical fiber](https://notes.aquiles.me/hollow_optical_fiber/), or faster cameras such as those used in [iNTA](https://notes.aquiles.me/literature/202212281528_refractive_index_measurement_of_evs_with_inta/) can help overcome some of the limitations.

#### Coulter Counter

A [Coulter counter](https://notes.aquiles.me/coulter_counter/) works by applying a voltage across two liquid reservoirs separated by a membrane with an aperture. Once a stedy current is established, every time an object flows through the aperture it will create an electronic blockage. At small scales, the aperture becomes a nano-pore, and the method was branded as [Tunable Resistive Pulse Sensing](https://notes.aquiles.me/tunable_resistive_pulse_sensing/) (TRPS).
...
In [Flow Cytometry](https://notes.aquiles.me/flow_cytometry/), the liquid containing the sample is passed through a special nozzle that creates a very narrow flow. A carefully aligned laser is then used to detect particles one by one as they flow through the focus spot.
...
The idea of a [nano flow cytometer](https://notes.aquiles.me/nanofcm/) is gaining a foothold in the community, but the control of the flow at the rates required is challenging, as well as the interpretation of the results.
...
[SP-IRIS](https://notes.aquiles.me/sp-iris/) is an optical method that relies on a two step process. First, a micro-patterned substrate is prepared with some antibodies. This is similar to an ELISA test, but with control on the number of attachment sites and their location on a _macroscopic_ area. The second step includes adding a fluorescent label, which adds an extra dimension to the detection events.

To count particles attached to a given site, the device uses an interferometric approach, not unlike [iSCAT](https://notes.aquiles.me/iscat/) (used in the [iNTA](https://notes.aquiles.me/inta/) approach). The interferometric detection allows to calculate the size of the particles, and the fluorescence signal (which can be multiplexed) yields the extra dimensions.

The biggest challenge with this approach is overcoming the [tyranny of the Langmuir binding isotherm](https://notes.aquiles.me/tyranny_of_the_langmuir_binding_isotherm/). To guarantee that small populations of particles are detected, the user needs to wait many hours (up to 24) for all the particles to have enough chances of interacting with the substrate.

#### Bulk methods and Electron Microscopes

Other approaches to measuring size include transforming sample preparation tools into analytical instruments. Such is the case for [analytical ultracentrifugation](https://notes.aquiles.me/analytical_ultracentrifugation/), that looks at the sedimentation properties of particles as the sample container spins, or [A4F-DLS-MALS](https://notes.aquiles.me/literature/202111301108_limitations_of_a4f-dls-mals/), which couples filtration with bulk size measurements ([dynamic light scattering](https://notes.aquiles.me/dynamic_light_scattering/), [multiangle light scattering](https://notes.aquiles.me/multiangle_light_scattering/)).
...
Single-molecule methods include strategies such as **STORM** or **PALM**, which became quite popular in the vesicle community in recent years. However, these methods are limited to a localization accuracy of around 20~50nm, which is not good enough to say much about the shape or size of particles below
...
I think in the coming couple of years we’ll see the emergence of diagnostic tests based on vesicles. In a first stage, these tests will be designed around existing tools and protocols. For example, an [elisa test](https://notes.aquiles.me/elisa_test/) after some sample enrichment.
...
[Machine learning](https://notes.aquiles.me/machine_learning/) algorithms able to analyze complex signals will naturally emerge as a necessary step.

### Missing pieces and challenges

Extracellular vesicles are a complex object to study. Part of the complexity is given by the shear amount pf particles in bodily fluids. If one would be looking at [EV’s from a tumor](https://notes.aquiles.me/evs_can_be_used_as_ovarian_cancer_markers/), for example, the chances of finding them amongst the myriad other EV’s and [lipoproteins](https://notes.aquiles.me/lipoproteins/) would be [close to zero](https://notes.aquiles.me/why_pushing_sensitivity_of_assays/).

That leads to the need to use **single-particle type of measurements**, where even **the smallest numbers can be detected**.

The biggest problem with small-number detections is the number of false-positives. There is still a lot of work to be done in labelling and capturing EV’s to limit the amount of mistakes. Label-free methods such as **[SERS](https://notes.aquiles.me/literature/202306221512_sers_to_identify_cancer_evs/) are very promising, but very hard to implement** under the high-throughput conditions necessary to create statistically sound results.

Combining techniques in simple to use devices will be crucial for extending the understanding and use of vesicles. Things such as **coupling electrophoretic mobility sorting with fluorescence** detection on the same particles, will be fundamental to unravel multi-dimensional analysis. Performing measurements on different devices has limited advantages, since the particles observed are not really _the same_.
