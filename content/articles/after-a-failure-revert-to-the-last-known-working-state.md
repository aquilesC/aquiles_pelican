...
Category: Personal
Tags: 
Summary: When facing failure in complex projects, reverting to the last known working state can help identify where things went wrong and provide a path forward.
...
## The challenge of multiparametric optimization
...
Overall, around 20 different parameters can be changed, and each one will impact the outcome. Optimizing for an abstract idea such as user-friendliness already generates an uncertain goal. Using a different laser simplifies the electronics, but the user will have to spend ten more minutes waiting for the device to be ready. How can you weigh the consequences of such a decision?
...
## Reverting to the last known working state
